...
JAR_NAME="Vulcan_Runner"  #如果是jar方式启动，必须与jar文件名相同
...
case $1 in
    start)
        if [ `jps | grep ${JAR_NAME}.jar  | wc -l` -gt 0 ];then
            echo "WARN: Vulcan_Runner service is already running, please stop it first!"
            exit -1
        fi
        #启动程序
        ...
        if [ `jps | grep ${JAR_NAME}.jar  | wc -l` -le 0 ];then
             echo "WARN: Vulcan_Runner service start failed, please check!"
             exit -1
        else
              echo "Vulcan_Runner service has started."
        fi
    ;;
    stop)
        jps | awk '{if($2=="'$JAR_NAME'.jar") print $1}' | xargs kill -15
        echo "Vulcan_Runner service has stopped."
    ;;
    status)
        if [ `jps | grep ${JAR_NAME}.jar | wc -l` -le 0 ];then
             echo "Vulcan_Runner service hasn't running."
             exit -1
        else
              echo "Vulcan_Runner service has running."
        fi
    ;;
    restart)
    ...
    ;;
    *)
        echo "Usage: sh Vulcan_Runner.sh {start|stop|status|restart}"
    ;;
esac
